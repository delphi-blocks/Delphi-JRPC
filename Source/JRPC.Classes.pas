...
type
  ...
  ///   in API classes, providing automatic memory management for
...
begin
  // Dynamic arrays (including arrays of objects, e.g. an array returned by a
  // method) are collected element by element, recursively, mirroring
  // CollectGarbageValue. A bare TValue array is not IsObject, so without this
  // the elements would leak.
  if AValue.IsArray then
  begin
    for var LIndex := 0 to AValue.GetArrayLength - 1 do
      Add(AValue.GetArrayElement(LIndex), AAction);
    Exit;
  end;
