...
resourcestring
  // Localizable messages for the JRPC layer (JRPC.Core/Classes/Server/Invoker)
...
type
  ...
  public
    ...
    ///   exception maps to, or nil when the error carries no structured data.
    /// </summary>
    /// <remarks>
    ///   The caller takes ownership of what comes back. The base implementation
    ///   returns nil, and the Data string below is used instead; only an error
    ///   whose specification defines a structured payload overrides this, and
    ///   when it does, the object wins over the string. A protocol layered on
    ///   JSON-RPC is the usual reason to need it - MCP defines object payloads
    ///   for several of its errors.
    /// </remarks>
  ...
  /// </summary>
  /// <remarks>
  ///   An explicitly empty path ([JRPCPath('')]) marks the class as "flat":
  ///   its methods declare the full JSON-RPC method name themselves
  ///   ([JRPCMethod('tools/call')]) instead of being addressed as
  ///   path + separator + method.
  /// </remarks>
  ...
  /// <summary>
  ///   Custom serializer for the TJRPCNotification class. Keeps the ownership of the "params" 
  ///   member explicit (clone in, no interaction with the RTL TJSONValueSerializer, which 
  ///   frees the value it replaces) and mirrors the request serializer.
  /// </summary>
  TJNotificationSerializer = class(TCustomSerializer)
  protected
    class function GetTargetInfo: PTypeInfo; override;
    class function CanHandle(AType: PTypeInfo): Boolean; override;
  public
  ...
  private
    FSeparator: string;
    /// <summary>
    ///   Index of the "flat" (fully qualified) method names declared by classes
    ///   marked with an empty path attribute. Non-owning: the proxies are owned
    ///   by the dictionary itself (doOwnsValues).
    /// </summary>
    FFlatMethods: TDictionary<string, TJRPCConstructorProxy>;
    function GetClassName(AClass: TClass): string;
    function IsFlatClass(AClass: TClass): Boolean;
    procedure AddFlatMethods(AClass: TClass; AProxy: TJRPCConstructorProxy);
    procedure RemoveFlatMethods(AProxy: TJRPCConstructorProxy);
  ...
  ///   itself, the response list, and any additional data associated with the
  ///   context.
  /// </summary>
...
begin
  Result := TNeonConfiguration.Camel
    ...
    .RegisterSerializer(TJNotificationSerializer)
...
begin
  ...
  begin
    ...
    // TJSONObject of an error "data" member is written as the properties of the
    // TJSONObject class - {"count":2,"isEmpty":false,...} - instead of as the
    // JSON it holds.
...
{ TJNotificationSerializer }

class function TJNotificationSerializer.CanHandle(AType: PTypeInfo): Boolean;
begin
  Result := TypeInfoIs(AType);
end;

function TJNotificationSerializer.Deserialize(AValue: TJSONValue; const AData: TValue; 
  ANeonObject: TNeonRttiObject; AContext: IDeserializerContext): TValue;
var
...
class function TJNotificationSerializer.GetTargetInfo: PTypeInfo;
begin
  Result := TJRPCNotification.ClassInfo;
end;
...
begin
  ...
  if IsFlatClass(AClass) then
    AddFlatMethods(AClass, AProxy);
end;

procedure TJRPCRegistry.AddFlatMethods(AClass: TClass; AProxy: TJRPCConstructorProxy);
var
  LMethod: TRttiMethod;
  LAttrib: JRPCAttribute;
  LOwner: TJRPCConstructorProxy;
begin
  // Methods are enumerated exactly like TJRPCInvoker.FindMethod does (GetMethods,
  // inherited methods included) so the index and the dispatch always agree.
  for LMethod in TRttiUtils.Context.GetType(AClass).GetMethods do
  begin
    LAttrib := TRttiUtils.FindAttribute<JRPCAttribute>(LMethod);
    if not Assigned(LAttrib) or LAttrib.Name.IsEmpty then
      Continue;

    if FFlatMethods.TryGetValue(LAttrib.Name, LOwner) then
      raise EJRPCException.CreateFmt(SJRPCDuplicateFlatMethod,
        [LAttrib.Name, LOwner.TypeTClass.QualifiedClassName]);

    FFlatMethods.Add(LAttrib.Name, AProxy);
  end;
end;

procedure TJRPCRegistry.RemoveFlatMethods(AProxy: TJRPCConstructorProxy);
var
  LPair: TPair<string, TJRPCConstructorProxy>;
  LNames: TArray<string>;
  LName: string;
begin
  LNames := [];
  for LPair in FFlatMethods do
    if LPair.Value = AProxy then
      LNames := LNames + [LPair.Key];

  for LName in LNames do
    FFlatMethods.Remove(LName);
end;
...
begin
  ...
  FSeparator := '/';
  FFlatMethods := TDictionary<string, TJRPCConstructorProxy>.Create;
end;

destructor TJRPCRegistry.Destroy;
begin
  FFlatMethods.Free;
  inherited;
end;

function TJRPCRegistry.GetClassInstance(const AName: string; out Value: TObject): Boolean;
var
...
begin
  ...
  begin
    ...
    // A flat class ([JRPCPath('')], or no attribute at all) is keyed by its
    // qualified name: it stays owned (and unregisterable) like any other class
    // without occupying the empty key, and it is reached through FFlatMethods.
...
function TJRPCRegistry.IsFlatClass(AClass: TClass): Boolean;
var
  LJRPCAttribute: JRPCAttribute;
begin
  // No path attribute at all, or an explicitly empty one: either way the class
  // contributes no prefix, so its [JRPCMethod] names are the full JSON-RPC
  // method names. Classes are registered explicitly, never discovered, so an
  // attribute is only needed when it actually says something (a path prefix, a
  // custom separator).
  LJRPCAttribute := TRttiUtils.FindAttribute<JRPCAttribute>(TRttiUtils.Context.GetType(AClass));
  Result := not Assigned(LJRPCAttribute) or LJRPCAttribute.Name.IsEmpty;
end;
...
begin
  ...
  // A flat method name ([JRPCPath('')] on the class, [JRPCMethod('tools/call')]
  // on the method) is fully qualified, so it wins over the prefix lookup below.
  // Nothing registered the old way can be shadowed by this: flat classes are
  // keyed by their qualified name, never by a path.
  if FFlatMethods.TryGetValue(AName, Value) then
    Exit(True);

  // A class may declare a custom separator (e.g. [JRPCPath('custom',
  // 'separator=.')]) which is recorded in the global FSeparator. Try it first
  // and fall back to the standard '/' separator so classes with a custom
  // separator can coexist with plain ones in the same registry.
  for LSeparator in [FSeparator, '/'] do
  begin
    LSeparatorIndex := Pos(LSeparator, AName);
    if LSeparatorIndex <= 0 then
      Continue;

    LClassName := Copy(AName, 0, LSeparatorIndex - 1);
    if TryGetValue(LClassName, Value) then
      Exit(True);
  end;

  // No separator: the whole name is the class name.
  Result := Self.TryGetValue(AName, Value);
end;
...
begin
  LName := GetClassName(AClass);
  // The flat entries must go first: Remove frees the proxy (doOwnsValues) and
  // FFlatMethods would be left with dangling references.
  if Self.TryGetValue(LName, LProxy) then
    RemoveFlatMethods(LProxy);
  Self.Remove(LName);
end;
...
begin
  ...
  begin
    ...
    // the member stays absent rather than turning up empty. A structured
    // payload wins over the string one: an error that defines an object shape
    // for "data" cannot say the same thing in a string.
    var LData := EJRPCException(E).CreateErrorData;
    if Assigned(LData) then
      Result.Error.Data := TValue.From<TJSONObject>(LData)
    else if EJRPCException(E).Data <> '' then
...
begin
  // Hook for hosts that contribute per-request data. The standalone JRPC
  // server has no plugin configuration system, so this is intentionally empty.
end;
...
begin
  ...
  // The request and the response list are also stored in the context data so
  // they can be injected into [Context]-annotated fields, not just exposed
  // through the CurrentRequest/Responses properties.
